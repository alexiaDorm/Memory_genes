...
scores_grid = []
for lamb in C:
    clf, scores = fit_evaluate_all(fused, 'svm', lamb = lamb, kernel = 'rbf', verbose = False)
    ...
    scores_df.to_csv('../data/binaryClass_scores/RbfSVMfused/' + str(lamb) + 'few.csv', index=True)
    
scores_df = pd.DataFrame(scores_grid, index = C, columns= scores_name)
scores_df.to_csv('../data/binaryClass_scores/RbfSVMfused/fusedfew.csv', index=True)